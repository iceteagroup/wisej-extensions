...
this.init = function (config) {
  ...
  this.__processCustomOptions(config);

  config.options.responsive = true;
  config.options.maintainAspectRatio = false;

  this.__processChartType(config);
  ...
  if (this.chart.config.data.datasets) {
	this.__processDataSets(this.chart.config.data.datasets);
	this.__processDataSetColors(this.chart.config.data.datasets);
  }
...
this.__processChartType = function (config) {

  if (config.type == "custom") {
	config.type = config.options.type;
...
this.__processPlugins = function (config) {
  ...
  // add the custom options to the plugins.
  // this is needed to support the custom plugin options that are not provided by the.NET API.
  if (plugins.customOptions) {

	// parse the custom options.
	var json = JSON.parse(plugins.customOptions);
	if (json) {

	  // add the custom options to the plugins.
	  config.options.plugins = Object.assign({}, config.options.plugins, json);

	  // remove the custom options from the plugins.
	  delete plugins.customOptions;
	}
  }
}

this.__processCustomOptions = function (config) {

  var options = config.options.customOptions;
  ...
  if (options) {
	...
	if (json) {
	  ...
	  // add the custom options to the chart.
	  config.options = Object.assign({}, config.options, json);
	}
	delete config.options.customOptions;
  }
}
...
this.__processAnimation = function (config) {
  for (var name in config) {
	if (this.__looksLikeFunction(config[name])) {
	  config[name] = eval(config[name]);
	}
  }
}

this.__processDataSetColors = function (datasets) {
  if (datasets instanceof Array && datasets.length > 0) {
	for (var i = 0; i < datasets.length; i++)
	  this.__setFontAndColors(datasets[i]);
  }
}
