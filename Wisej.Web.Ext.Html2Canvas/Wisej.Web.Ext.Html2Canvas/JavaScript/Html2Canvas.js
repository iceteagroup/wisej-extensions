		...
		var parseBackgroundImages = function parseBackgroundImages(style, resourceLoader) {
			...
			return sources.map(function (source, index) {
				...
				if (source.args.length > 0) {
					for (var i = 0; i < source.args.length; i++) {
						if (source.args[i] && isSVG(source.args[i])) {
							if (size[i].size == 0)
								size[i].size = BACKGROUND_SIZE.CONTAIN;
						}
					}
				}
		...
		var CanvasRenderer = function () {
			...
			_createClass(CanvasRenderer, [{
				...
				value: function drawImage(image, source, destination) {
					...
					this.ctx.drawImage(image, destination.left, destination.top, destination.width, destination.height);
				}
			}, {
				key: 'drawShape',
